...
import { TrainingService } from 'app/pages/trainer/trainer-services/trainer.service'; 
...
export class AttendanceComponent implements OnInit {
 ...
 public filteredData:TableRow[]=[];
  ...
  // }
  fetchTrainings(empName: string) {
    const url = `http://localhost:8083/api/training-history/trainer-ongoing/${this.trainerName}/${empName}`;

    this.http.get<Trainings[]>(url).subscribe(
        (response) => {
            console.log('Training Data: ', response);
            this.originalData = response
                .filter(item => item.training_status.toLowerCase() === 'on-going') // Update here
                .map((item, index) => ({
                    number: (index + 1).toString(),
                    course: item.course,
                    plannedStartDate: this.formatDate(item.plannedStartDate),
